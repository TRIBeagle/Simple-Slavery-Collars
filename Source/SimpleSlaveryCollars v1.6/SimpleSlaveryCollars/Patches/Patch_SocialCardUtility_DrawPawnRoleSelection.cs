...
namespace SimpleSlaveryCollars.Patches
{
    ...
    public static class Patch_SocialCardUtility_DrawPawnRoleSelection
    {
        ...
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
        {
            ...
            if (replacement == null)
            {
                Log.Warning("[SSC] Transpiler: IsEligibleForRoleSelection method not found. Skipping patch.");
                foreach (var inst in instructions)
                    yield return inst;
                yield break;
            }
            ...
            if (!patched)
                Log.Warning("[SSC] Transpiler: IsFreeNonSlaveColonist call not found in DrawPawnRoleSelection.");
        }
