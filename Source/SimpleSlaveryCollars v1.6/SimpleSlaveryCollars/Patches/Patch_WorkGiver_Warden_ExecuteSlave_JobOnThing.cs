// SimpleSlaveryCollars | Patches | Patch_WorkGiver_Warden_ExecuteSlave_JobOnThing.cs
// 목적   : Warden이 자기 자신(Pawn == TargetPawn)을 노예 처형 대상으로 잘못 선택하는 경우를 차단
...
{
    /// <summary>
    /// WorkGiver_Warden_ExecuteSlave.JobOnThing Prefix 패치.
    /// - Warden이 자기 자신을 노예 처형 대상으로 잡는 경우를 무효화한다.
    /// </summary>
    ...
    public static class Patch_WorkGiver_Warden_ExecuteSlave_JobOnThing
    {
        static bool Prefix(Pawn pawn, Thing t, ref Job __result)
        {
            ...
            {
                Log.Error($"[SSC] Patch_WorkGiver_Warden_ExecuteSlave_JobOnThing.Prefix 오류: {ex}");
            }
            return true;
        }
    }
